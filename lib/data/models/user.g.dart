...
const UserSchema = CollectionSchema(
  ...
  idName: r'id',
  indexes: {
    r'email_index': IndexSchema(
      id: 8392687653683471638,
      name: r'email_index',
      unique: true,
      replace: false,
      properties: [
...
extension UserByIndex on IsarCollection<User> {
  Future<User?> getByEmail(String? email) {
    return getByIndex(r'email_index', [email]);
  }

  User? getByEmailSync(String? email) {
    return getByIndexSync(r'email_index', [email]);
  }

  Future<bool> deleteByEmail(String? email) {
    return deleteByIndex(r'email_index', [email]);
  }

  bool deleteByEmailSync(String? email) {
    return deleteByIndexSync(r'email_index', [email]);
  }

  Future<List<User?>> getAllByEmail(List<String?> emailValues) {
    final values = emailValues.map((e) => [e]).toList();
    return getAllByIndex(r'email_index', values);
  }

  List<User?> getAllByEmailSync(List<String?> emailValues) {
    final values = emailValues.map((e) => [e]).toList();
    return getAllByIndexSync(r'email_index', values);
  }

  Future<int> deleteAllByEmail(List<String?> emailValues) {
    final values = emailValues.map((e) => [e]).toList();
    return deleteAllByIndex(r'email_index', values);
  }

  int deleteAllByEmailSync(List<String?> emailValues) {
    final values = emailValues.map((e) => [e]).toList();
    return deleteAllByIndexSync(r'email_index', values);
  }

  Future<Id> putByEmail(User object) {
    return putByIndex(r'email_index', object);
  }

  Id putByEmailSync(User object, {bool saveLinks = true}) {
    return putByIndexSync(r'email_index', object, saveLinks: saveLinks);
  }

  Future<List<Id>> putAllByEmail(List<User> objects) {
    return putAllByIndex(r'email_index', objects);
  }

  List<Id> putAllByEmailSync(List<User> objects, {bool saveLinks = true}) {
    return putAllByIndexSync(r'email_index', objects, saveLinks: saveLinks);
  }
}
